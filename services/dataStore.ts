import { Material, CutJob, PricingConfig, ReorderAlert, MeasurementUnit } from '../types';
...
export function getUserMeasurementUnit(): MeasurementUnit {
  const stored = localStorage.getItem('user_measurement_unit');
  return (stored as MeasurementUnit) || 'imperial';
}

// Set user's preferred measurement unit
export function setUserMeasurementUnit(unit: MeasurementUnit): void {
  localStorage.setItem('user_measurement_unit', unit);
}
